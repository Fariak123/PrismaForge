...
export function prismaToSnapshot(schema: ParsedPrismaSchema): SchemaSnapshot {
  const tableMap = new Map<string, string>();

  const columnMap = new Map<string, string>();

  const tables = schema.models.map((model, index) => {
    const tableId = crypto.randomUUID();

    tableMap.set(model.name, tableId);

    const columns = model.fields.map((field) => {
      const columnId = crypto.randomUUID();

      columnMap.set(`${model.name}.${field.name}`, columnId);
      ...
      return {
        ...
        unique: field.isUnique || field.isId,
      };
    });
  ...
  const relationships = createRelationships(schema, tableMap, columnMap);
  return {
    tables,
    relationships,
  };
...
function createRelationships(
  ...
  for (const model of schema.models) {
    for (const relation of model.relations) {
      ...
      // Find FK side
      // Post.user -> User
      const fkRelation = targetModel.relations.find(
        (r) => r.targetModel === model.name && r.sourceFields.length > 0,
      );

      if (!fkRelation) continue;

      const sourceTableId = tableMap.get(model.name);

      const targetTableId = tableMap.get(targetModel.name);
...
function mapPrismaType(type: string): DataType {
  switch (type) {
    case 'String':
      return 'String';
