...
function generateRelationField(
  ...
  const isSource = relationship.sourceTableId === table.id;

  const otherTable = tableMap.get(
    isSource ? relationship.targetTableId : relationship.sourceTableId,
  );
  ...
  const otherName = otherTable.name;
  ...
  */
  ...
  if (relationship.type === 'one-to-many') {
    if (isSource) {
      return `  ${otherName.toLowerCase()}s ${otherName}[]\n`;
    }

    const fkColumn = findColumn(table, relationship.targetColumnId);
...
function generateModel(
  ...
  for (const relationship of relationships) {
    if (
      ...
      output += generateRelationField(table, relationship, tableMap);
    }
  }
...
export function generatePrisma(tables: Table[], relationships: Relationship[]) {
  const tableMap = new Map(tables.map((table) => [table.id, table]));

  const models = tables
    .map((table) => generateModel(table, relationships, tableMap))
    .join('\n');
